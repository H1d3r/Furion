...
public static class IEnumerableExtensions
{
    ...
    /// <typeparam name="TSource">泛型类型</typeparam>
    /// <param name="sources">集合对象</param>
    /// <param name="conditionExpressions">条件表达式元组数组</param>
    /// <returns>新的集合对象</returns>
    ...
    {
        if (conditionExpressions == null || conditionExpressions.Length == 0) return sources;

        var expressions = conditionExpressions
            .Where(t => t.condition && t.expression != null)
            .Select(t => t.expression)
            .ToArray();

        return expressions.Length == 0 ? sources : sources.Where(expressions);
    }
